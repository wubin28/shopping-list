...
async function loadShoppingItems() {
  try {
    const response = await axios.get('http://%%API_URL%%:8081/api/v1/shopping-items')
...
async function createShoppingItem(shoppingItem: ShoppingItem) {
  ...
  await axios.post('http://%%API_URL%%:8081/api/v1/shopping-items', {
...
async function updateShoppingItem(shoppingItem: ShoppingItem) {
  console.log('ShoppingItem', shoppingItem)
  await axios.put(`http://%%API_URL%%:8081/api/v1/shopping-items/${shoppingItem.id}`, {
    id: shoppingItem.id,
    item: shoppingItem.item,
    purchased: shoppingItem.purchased
  })
...
async function deleteShoppingItemById(shoppingItem: ShoppingItem) {
  await axios.delete(`http://%%API_URL%%:8081/api/v1/shopping-items/${shoppingItem.id}`)
  ElMessage({
